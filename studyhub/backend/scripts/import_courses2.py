...
def create_question_bank(db, course, data, created_by_id=None):
    ...
    for q_data in questions_data:
        ...
        # Create the question
        question = Question(
            id=question_id,  # Use provided ID or let database assign one
            title=title,
            content=content,
            type=q_type,
...
def create_assignments_from_json(db, course, data, question_map):
    ...
    # Get all questions by week for fallback if question_ids are missing
    week_questions = {}
    ...
    for a_data in assignments_data:
        ...
        week_id = a_data.get("week_id")
        
        if not title or not week_id:
            log(f"  Missing required fields for assignment: {a_data}", "ERROR")
            continue
        
        # Get week
        week = db.query(Week).filter_by(id=week_id, course_id=course.id).first()
        if not week:
            week = db.query(Week).filter_by(number=week_id, course_id=course.id).first()
        
        if not week:
            log(f"  Week {week_id} not found for assignment {title}", "ERROR")
            continue
...
def process_single_file(db_session, json_file, admin):
    ...
    try:
        ...
        if code in existing_courses:
            ...
            # Process question bank first (for new or updated questions)
            log("Creating/updating question bank...")
            question_map = create_question_bank(db_session, course, data, admin.id)
            
            # Process assignments (link to questions)
            log("Creating/updating assignments...")
            assignments = create_assignments_from_json(db_session, course, data, question_map)
            
            # Process weeks and lectures
            weeks_data = data.get("weeks", [])
            if weeks_data:
                log(f"Updating {len(weeks_data)} weeks for course {code}")
                process_weeks(db_session, course, data)
