...
class DocumentIndexer:
    ...
    def _ensure_general_collection(self):
    ...
    def index_document(self, document_id: str) -> None:
        ...
        try:
            ...
            collection_name = status["metadata"].get("collection", "general")
    ...
    def search(
        ...
        try:
            ...
            # Prepare filters
            where_clause = filters if filters and len(filters) > 0 else {"$and": [{"document_id": {"$exists": True}}]}
            
            # Get total count first (with filters but no limit)
            total_results = self.chroma.count_matches(
                collection_name=collection_name,
    ...
    def _update_status(
        ...
        status_path = os.path.join(settings.PROCESSED_DIR, f"{document_id}.json")
        current_status = self.get_document_status(document_id)
